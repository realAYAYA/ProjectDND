...
void FJsEnv::SetJsExceptionRaisedCallback(std::function<void(const char*, const FString&)> Callback)
{
	GameScript->SetJsExceptionRaisedCallback(Callback);
}

void FJsEnv::GetHeapStatistics(FJsHeapStatistics& Out)
{
	GameScript->GetHeapStatistics(Out);
}
