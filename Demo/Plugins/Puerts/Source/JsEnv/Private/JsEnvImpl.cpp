    ...
    if (!BindInfoPtr || ForceReinject)    // create and link
    {
        ...
        if (PrefixPos != INDEX_NONE)
        {
            ...
            if (TryCatch.HasCaught())
            {
            	...
            	OnJsExceptionRaised(__FUNCTION__, TryCatch, ErrorMessage);
                return;
            }
...
void FJsEnvImpl::JsHotReload(FName ModuleName, const FString& JsSource)
{
    ...
    if (ModuleLoader->Search(TEXT(""), ModuleName.ToString(), OutPath, OutDebugPath))
    {
        ...
        if (TryCatch.HasCaught())
        {
            FString ErrorMessage = FString::Printf(TEXT("reload module exception %s"), *FV8Utils::TryCatchToString(Isolate, &TryCatch));
        	OnJsExceptionRaised(__FUNCTION__, TryCatch, ErrorMessage);
        }
    }
    else
...
void FJsEnvImpl::SetJsExceptionRaisedCallback(std::function<void(const char*, const FString&)> Callback)
{
	OnJsExceptionRaisedCallback = Callback;
}

void FJsEnvImpl::GetHeapStatistics(FJsHeapStatistics& Out)
{
	...
	Out.TotalHeapSize = Statistics.total_heap_size();
	Out.TotalHeapSizeExecutable = Statistics.total_heap_size_executable();
	Out.TotalPhysicalSize = Statistics.total_physical_size();
	Out.TotalAvailableSize = Statistics.total_available_size();
	Out.TotalGlobalHandlesSize = Statistics.total_global_handles_size();
	Out.UsedGlobalHandlesSize = Statistics.used_global_handles_size();
	Out.UsedHeapSize = Statistics.used_heap_size();
	Out.HeapSizeLimit = Statistics.heap_size_limit();
	Out.MallocedMemory = Statistics.malloced_memory();
	Out.ExternalMemory = Statistics.external_memory();
	Out.PeakMallocedMemory = Statistics.peak_malloced_memory();
	Out.NumberOfNativeContexts = Statistics.number_of_native_contexts();
	Out.NumberOfDetachedContexts = Statistics.number_of_detached_contexts();
	Out.DoesZapGarbage = Statistics.does_zap_garbage();
}

void FJsEnvImpl::OnJsExceptionRaised(const char* FunctionName, const v8::TryCatch& TryCatch, const FString& Message)
{
	Logger->Error(Message);
	if (OnJsExceptionRaisedCallback)
	{
		OnJsExceptionRaisedCallback(FunctionName, Message);
	}
}
            ...
            if (UNLIKELY(IsCDO))
            {
                if (TypeScriptGeneratedClass->IsChildOf(UBlueprintFunctionLibrary::StaticClass()))
                {
                         ...
                         ++FuncIt)
                    {
                    	...
                    	Function->FunctionFlags |= FUNC_Static;  // hudawei: 蓝图函数库的所有函数必然是静态的
...
void FJsEnvImpl::InvokeDelegateCallback(UDynamicDelegateProxy* Proxy, void* Params)
{
    ...
    if (TryCatch.HasCaught())
    {
        FString Message = FString::Printf(TEXT("js callback exception %s"), *FV8Utils::TryCatchToString(Isolate, &TryCatch));
    	OnJsExceptionRaised(__FUNCTION__, TryCatch, Message);
    }
}
...
void FJsEnvImpl::InvokeJsMethod(UObject* ContextObject, UJSGeneratedFunction* Function, FFrame& Stack, void* RESULT_PARAM)
{
    ...
    if (TryCatch.HasCaught())
    {
        FString Message = FString::Printf(TEXT("call %s::%s of %p fail: %s"), *ContextObject->GetClass()->GetName(),
...
void FJsEnvImpl::Start(const FString& ModuleNameOrScript, const TArray<TPair<FString, UObject*>>& Arguments, bool IsScript)
{
    ...
    if (IsScript)
    {
        ...
        if (TryCatch.HasCaught())
        {
            FString Message = FV8Utils::TryCatchToString(Isolate, &TryCatch);
        	OnJsExceptionRaised(__FUNCTION__, TryCatch, Message);
        }
    }
    else
