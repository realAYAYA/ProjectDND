...
void FFunctionTranslator::Init(UFunction* InFunction, bool IsDelegate)
{
    ...
    for (TFieldIterator<PropertyMacro> It(InFunction); It && (It->PropertyFlags & CPF_Parm); ++It)
    {
        ...
        if (IsStatic && !InFunction->IsNative() && Property->GetFName() == WorldContextPinName)
        {
        	HasWorldContextInArg = true;
        	if (Arguments.size() == 0)
        	{
    ...
    v8::Local<v8::Value> This, UObject* ContextObject, FFrame& Stack, void* RESULT_PARAM)
{
    ...
    if (!Result.IsEmpty())    // empty mean exception
    {
    	...
    	int32 OutIdx = Arguments.size() - OutArgsNum - 1;
    	if (SkipWorldContextInArg0)
			OutIdx += 1;
    	if (OutIdx < 0)
    		OutIdx = 0;
        ...
        for (int i = 0; i < Arguments.size(); ++i)
        {
            if (Arguments[i]->IsOut())
            {
                ...
                if (OutParmRec)
                {
                	if (HasWorldContextInArg)
                	{
                		Arguments[i]->JsToUEOut(Isolate, Context, Args[OutIdx], OutParmRec->PropAddr, true);
                	}
	                else
	                {
