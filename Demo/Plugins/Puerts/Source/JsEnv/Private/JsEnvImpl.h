...
public:
	...
	virtual void SetJsExceptionRaisedCallback(std::function<void(const char*, const FString&)> Callback) override;
	virtual void GetHeapStatistics(FJsHeapStatistics& Out) override;
	
	void OnJsExceptionRaised(const char* FunctionName, const v8::TryCatch& TryCatch, const FString& Message);

	std::function<void(const char*, const FString&)> OnJsExceptionRaisedCallback;
