...
void FTypeScriptDeclarationGenerator::Begin(FString ModuleName)
{
	...
	if (!bManualGenerate)
	{
		Output << "/// <reference path=\"zprotocol.d.ts\" />\n";
	}
...
void FTypeScriptDeclarationGenerator::Gen(UObject* ToGen)
{
	if (!bManualGenerate)
	{
		if (auto* Package = ToGen->GetPackage())
		{
			// hudawei: 跳过 ZProtocol ， 该模块的类型定义拆分至 zprotocol.d.ts 另外生成 
			const FString& PackageName = Package->GetName(); 
			if (PackageName == TEXT("/Script/ZProtocol"))
			{
				const FString& ObjectName = ToGen->GetName();
				if (ObjectName.StartsWith(TEXT("Z")) || ObjectName.StartsWith(TEXT("EZ")))
				{
					// 为了兼容性，C++代码符号还是生成	
				}
				else
	...
	if (ToGen->GetPathName().StartsWith(TEXT("/Game/Developers/")))
	{
		UE_LOG(LogTemp, Display, TEXT("Puerts GenIgnore %s"), GetData(ToGen->GetPathName()));
		return;  // hudawei: 测试相关资产不生成
	}
